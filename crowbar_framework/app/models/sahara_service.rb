...
class saharaService < ServiceObject
  ...
  def create_proposal
    ...
    controllers = select_nodes_for_role(
      nodes, "sahara-server", "controller") || []

    base["deployment"][@bc_name]["elements"] = {
      "sahara-server" => controllers.empty? ? [] : [controllers.first.name]
    }
  ...
  def validate_proposal_after_save(proposal)
    ...
    validate_at_least_n_for_role proposal, "sahara-server", 1
    super
  end
