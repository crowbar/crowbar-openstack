...
class WatcherService < OpenstackServiceObject
  ...
  def create_proposal
    ...
    if nodes.size >= 1
      ...
      base["deployment"][@bc_name]["elements"] = {
        "watcher-decision-engine" => [controller[:fqdn]],
        "watcher-applier" => [controller[:fqdn]],
        "watcher-api" => [controller[:fqdn]]
      }
    end
  ...
  def validate_proposal_after_save(proposal)
    validate_one_for_role proposal, "watcher-api"
    validate_one_for_role proposal, "watcher-decision-engine"
    validate_one_for_role proposal, "watcher-applier"

    super
  end

  def apply_role_pre_chef_call(old_role, role, all_nodes)
    @logger.debug("#{@bc_name} apply_role_pre_chef_call: entering #{all_nodes.inspect}")
    return if all_nodes.empty?

    vip_networks = ["admin", "public"]

    server_elements, server_nodes, ha_enabled = role_expand_elements(role, "wacher-api")
