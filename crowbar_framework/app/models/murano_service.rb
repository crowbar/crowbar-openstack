...
class MuranoService < PacemakerServiceObject
  ...
  def create_proposal
    @logger.debug("murano create_proposal: entering")
    base = super
    ...
    if nodes.size >= 1
      ...
      base["deployment"][@bc_name]["elements"] = { "murano-server" => [controller[:fqdn]] }
    end
    ...
    @logger.debug("murano create_proposal: exiting")
    base
  end

  def validate_proposal_after_save(proposal)
    validate_one_for_role proposal, "murano-server"
    validate_at_least_n_for_role proposal, "murano-server", 1
    super
  end

  def apply_role_pre_chef_call(old_role, role, all_nodes)
    @logger.debug("murano apply_role_pre_chef_call: entering #{all_nodes.inspect}")
    return if all_nodes.empty?

    vip_networks = ["admin", "public"]

    server_elements, server_nodes, ha_enabled = role_expand_elements(role, "murano-server")
    reset_sync_marks_on_clusters_founders(server_elements)
    Openstack::HA.set_controller_role(server_nodes) if ha_enabled

    role.save if prepare_role_for_ha_with_haproxy(role, ["murano", "ha", "enabled"],
    ...
    @logger.debug("murano apply_role_pre_chef_call: leaving")
  end
end
