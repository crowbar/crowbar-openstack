...
haproxy_loadbalancer "aodh-api" do
  address "0.0.0.0"
  port node[:ceilometer][:aodh][:api][:port]
  use_ssl false
  # FIXME: this is wrong, we must look into [:ceilometer][:aodh][:ha][:api] !
...
["api", "evaluator", "notifier", "listener"].each do |service|
  primitive_name = "aodh-#{service}"

  pacemaker_primitive primitive_name do
    agent node[:ceilometer][:aodh][:ha][service.to_sym][:agent]
    op node[:ceilometer][:aodh][:ha][service.to_sym][:op]
    action :update
