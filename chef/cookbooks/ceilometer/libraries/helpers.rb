  ...
  class << self
    def keystone_settings(node)
      ...
      if @keystone_settings.nil?
        ...
        nodes = []
        Chef::Search::Query.new.search(:node, "roles:keystone-server AND keystone_config_environment:keystone-config-#{node[:ceilometer][:keystone_instance]}") { |o| nodes << o }
        if nodes.empty?
          keystone_node = node
        else
        ...
        @keystone_settings['service_user'] = node[:ceilometer][:keystone_service_user]
        @keystone_settings['service_password'] = node[:ceilometer][:keystone_service_password]
