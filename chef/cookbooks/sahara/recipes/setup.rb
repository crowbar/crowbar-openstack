...
trustee_domain_name = node["sahara"]["trustee"]["domain_name"]
trustee_domain_admin = node["sahara"]["trustee"]["domain_admin_name"]
trustee_domain_admin_password = node["sahara"]["trustee"]["domain_admin_password"]

insecure = keystone_settings["insecure"] ? "--insecure" : ""

bash "register sahara domain" do
  user "root"
  code <<-EOF
    id=
    eval $(openstack #{insecure} domain create \
        #{trustee_domain_name} \
        --description 'Owns users and projects created by sahara' \
    ...
    eval $(openstack #{insecure} \
        ...
        --description "Manages users and projects created by sahara" \
        #{trustee_domain_admin})
    ret=
...
ruby_block "Update node parameters" do
  block do
    ...
    node.set[:sahara][:trustee][:domain_id] = trustee_domain_id
    node.set[:sahara][:trustee][:domain_admin_id] = trustee_domain_admin_id
