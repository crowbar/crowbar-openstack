if node[:nova][:trusted_flavors]
 flavors={11=>
  ...
  novacmd = "nova --os-username #{keystone_settings['service_user']} --os-password #{keystone_settings['service_password']} --os-tenant-name #{keystone_settings['service_tenant']} --os-auth-url #{keystone_settings['internal_auth_url']} --os-endpoint-type internalURL --os-region-name '#{keystone_settings['endpoint_region']}'"
  if ssl_insecure
