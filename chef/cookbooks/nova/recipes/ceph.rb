...
cinder_controller[:cinder][:volumes].each_with_index do |volume, volid|
  ...
  if volume[:rbd][:use_crowbar]
    ...
    # test to see if the user keyring exists and works
    # otherwise test to see if the admin keyring works
    if !client_keyring.empty? && File.exists?(client_keyring)
      ...
      unless check_ceph.run_command.stdout.match("(HEALTH_OK|HEALTH_WARN)")
        Chef::Log.info("Ceph client keyring can't talk to ceph cluster;  " \
                       "Nova skipping the ceph setup for backend : " \
                       "#{volume[:backend_name]}")
        next
      end
    else
