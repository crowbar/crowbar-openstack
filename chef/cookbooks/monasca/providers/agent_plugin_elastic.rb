...
action :create do
  ...
  node[:monasca][:agent_plugin_config][:elastic_instances] ||= {}

  # add the hash with the given values
  node[:monasca][:agent_plugin_config][:elastic_instances][new_resource.built_by] =
  ...
  elastic_instances = node[:monasca][:agent_plugin_config][:elastic_instances]
  ...
  elastic_conf = JSON.parse({ "init_config" => nil,
                              "instances" => elastic_instances.values }.to_json).to_yaml

  # write http_check plugin config
  file "/etc/monasca/agent/conf.d/elastic.yaml" do
